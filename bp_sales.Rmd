...
sales.1 <- dplyr::select(sales, price, area, fullrooms, halfrooms, varos, floor, lift, balcony)
...
training <- sales.1[inTrain,]
testing <-  sales.1[-inTrain,]

nrow(training)
nrow(na.exclude(training))
nrow(testing)
nrow(na.exclude(testing))

# Randomly split the data into training and testing sets (based on the preductors)
##
##
...
lm_fit <- caret::train(price ~ .,
                       data = na.exclude(training),
                       method = "lm",
                       tuneLength = 15,
                       trControl = ctrl,
...
price_pred_lm <- predict(lm_fit, newdata=testing)
summary(price_pred_lm)

# performance measure for regression
postResample(pred = price_pred_lm, obs = na.exclude(testing$price))

             
             
# Calculate error on testing set
errTest = (price_pred_lm - testing$price)/testing$price * 100
summary(errTest)
...
trainX <-trainSet[,2:length(sales.c1.trans)]        # Pull out the variables for training
...
```{r}
# select the features of interest
sales.1 <- dplyr::select(sales, price, area, fullrooms, halfrooms, varos, floor, storeys, lift, heating, balcony)

# skewness
e1071::skewness(sales.c1$price, na.rm = T)
e1071::skewness(sales.c1$area, na.rm = T)
e1071::skewness(sales.c1$halfrooms,na.rm = TRUE)

# define the transformation to remove skewness, center and scale the predictors
trans <- caret::preProcess(sales.c1[-1],
                           method = c("BoxCox", "center", "scale"))

# apply the transformation to the predictors
sales.c1.trans <- predict(trans, sales.c1[-1])
head(sales.c1.trans)
...
caret::nearZeroVar(sales.c1.trans)
...
```{r}
sales.c1.trans <- cbind(price = sales.c1$price, sales.c1.trans)
str(sales.c1.trans)
