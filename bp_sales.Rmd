...
sales.1 <- dplyr::select(sales, price, area, fullrooms, halfrooms, varos, condition, floor, lift, heating, view, lat, long, balcony, utility)

# Near-zero variables
nzv <- nearZeroVar(sales.1, saveMetrics= TRUE)
print(nzv)

# Impute missing data to the training set
summary(sales.1$balcony)
sales.1$balcony <- ifelse(is.na(sales.1$balcony), 0, sales.1$balcony)
...
NROW(na.exclude(training.1))
NROW(na.exclude(testing.1))

```
### Baseline model: price ~ varos + area
#### Simple lookup
The baseline model is a simple one: the average or median price per square meter in each "varos".
```{r}
# select the features of interest
sales.0 <- dplyr::select(sales, price, ppsm, area, varos)
sales.0 <- dplyr::filter(sales.0, area >=30 & area <= 100)
sales.0 <- dplyr::filter(sales.0, price <= 100)

# Randomly split the data into training and testing sets
set.seed(101)
  ## based on outcome
  inTrain <- caret::createDataPartition(sales.0$price, p=0.75,list = FALSE)
  ## based on predictors
  # index <- maxDissim()
training.0 <- na.exclude(sales.0[inTrain,])
testing.0 <-  na.exclude(sales.0[-inTrain,])

NROW(training.0)
NROW(testing.0)

# create a separate data frame that splits the original data frame by district/varos
vaross <- dplyr::group_by(training.0, varos)

# build model on training set (mean and median)
bl <- dplyr::summarise(vaross, 
                 total.count = n(),
                 avg.ppsm = mean(ppsm, na.rm=TRUE)*1000,
                 median.ppsm = median(ppsm, na.rm=TRUE)*1000)
print(bl)

# use a joint to mimic a VLOOKUP function and make predictions on testing set
testing.0 <- inner_join(testing.0, bl, by=c("varos"))
testing.0 <- dplyr::mutate(testing.0, price.pred = median.ppsm * area /1000)

# pred and obs for the testing set
pred.test <- testing.0$price.pred
obs.test <- testing.0$price

# Plot residuals vs. observed values
kernlab::plot(obs.test, (pred.test-obs.test) / obs.test*100)

# Plot predicted values vs. observed values
kernlab::plot(obs.test,pred.test)

# Performance measure for regression
caret::postResample(pred = pred.test , obs = obs.test)
...
#### Linear regression
```{r}
price_mod_lm.bl <- caret::train(price ~ area + varos,
                               data = training.0,
                               na.action = na.omit,
                               method = "lm",
                               tuneLength = 15)
...
# model summary                       
summary(price_mod_lm.bl) 

# Predictions on testing set
price_pred_lm.bl <- predict(price_mod_lm.bl, newdata=testing.0)

# pred and obs for the testing set
pred.test <- price_pred_lm.bl
obs.test <- testing.0$price
...
# Calculate error on testing set
errTest = (pred.test - obs.test)/obs.test * 100
summary(errTest)
...
cpGrid = expand.grid( .cp = seq(1e-10,1e-1,1e-1)) 
...
# Create a CART model
price_mod_rpart <- rpart::rpart(price ~ ., data = training.1, cp = 1e-10)
...
# Perform the cross validation
set.seed(7)
mtryGrid <- expand.grid(.mtry=c(1:12))
...
## Model for simple AirBnB optimisation
For comparison with AirBnB data, let's look at the variations per location then size:
```{r}

table(sales$fullrooms)

# create a separate data frame that splits the original data frame by district/varos
airBnB <- dplyr::group_by(sales, district, fullrooms)

dplyr::summarise(airBnB, 
                 total.count = n(),
                 avg.price = round(mean(price, na.rm=TRUE), 1),                 
                 median.price = round(median(price, na.rm=TRUE), 1),
                 sd.price = round(sd(price, na.rm=TRUE), 1))
```

Use caret with LM. Transform predictors only.
```{r}
# prepare the dataset
sales.20 <- dplyr::select(sales, price, area, fullrooms, halfrooms, district, varos, condition, floor, lift, heating, view, lat, long, balcony, utility)

# Randomly split the data into training and testing sets
set.seed(101)
  ## based on outcome
  inTrain <- caret::createDataPartition(sales.20$price, p=0.75,list = FALSE)
  ## based on predictors
  # index <- maxDissim()
training.20 <- sales.20[inTrain,]
testing.20 <-  sales.20[-inTrain,]
...
price_mod_lm.airBnB <- caret::train(price ~  district + fullrooms,
                               data = training.20,
                               na.action = na.omit,
                               method = "lm",
                               tuneLength = 15,
...
# model summary                       
summary(price_mod_lm.airBnB) 

# Plot residuals vs. observed values
kernlab::plot(training.20$price, resid(price_mod_lm.airBnB) / training.20$price*100)

# Plot predicted values vs. observed values
kernlab::plot(training.20$price,fitted(price_mod_lm.airBnB))

# Predictions on testing set
testing.20 <- na.exclude(testing.20)
nrow(testing.20)
price_pred_lm.airBnB <- predict(price_mod_lm.airBnB, newdata=testing.20)

# Performance measure for regression
caret::postResample(pred = price_pred_lm.airBnB, obs = testing.20$price)

# Calculate error on testing set
errTest = (price_pred_lm.airBnB - testing.20$price)/testing.20$price * 100
summary(errTest)
