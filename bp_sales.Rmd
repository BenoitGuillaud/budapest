...
### Prepare the datasets with dummy variables
```{r}
# Dummify all the (factor) variables in the dataset with "~ ." with fullRank = T for LM
dmy <- dummyVars("~ .", data = sales.1, fullRank = T)
sales.10 <- data.frame(predict(dmy, newdata = sales.1))

# Randomly split the data into training and testing sets (based on the outcome)
set.seed(101)
inTrain <- caret::createDataPartition(sales.10$price, p=0.75,list = FALSE)
training.10 <- sales.10[inTrain,]
testing.10 <-  sales.10[-inTrain,]


# Dummify all the (factor) variables in the dataset with "~ ." with fullRank = F for CART
dmy <- dummyVars("~ .", data = sales.1, fullRank = F)
sales.11 <- data.frame(predict(dmy, newdata = sales.1))

# Randomly split the data into training and testing sets (based on the outcome)
set.seed(101)
inTrain <- caret::createDataPartition(sales.11$price, p=0.75,list = FALSE)
training.11 <- sales.11[inTrain,]
testing.11 <-  sales.11[-inTrain,]
```
...
price_mod_lm.A.dum <- caret::train(price ~ .,
                               data = na.exclude(training.10),
                               method = "lm",
                               tuneLength = 15,
                               trControl = ctrl,
                               preProc = c("center", "scale"))
...
# model summary                       
summary(price_mod_lm.A.dum) 

# Plot residuals vs. observed values
kernlab::plot(na.exclude(training.10)$price, resid(price_mod_lm.A.dum)/na.exclude(training.10)$price*100)

# Plot predicted values vs. observed values
kernlab::plot(na.exclude(training.10)$price,fitted(price_mod_lm.A.dum))

# Predictions on testing set
testing.10 <- na.exclude(testing.10)
price_pred_lm.A.dum <- predict(price_mod_lm.A.dum, newdata=testing.10)

# Performance measure for regression
caret::postResample(pred = price_pred_lm.A.dum, obs = testing.10$price)

# Calculate error on testing set
errTest = (price_pred_lm.A.dum - testing.10$price)/testing.10$price * 100
summary(errTest)
...
```
Summary:
  - Dummifying the factor variables has no effect on the linear regression
...
cpGrid = expand.grid( .cp = seq(1e-8,1e-6,1e-8)) 

# Perform the cross validation
price_cv_rpart.dum <- caret::train(price ~ .,
                               data = training.11,
                               na.action = na.omit,
                               method = "rpart",
                               trControl = ctrl,
...
# Cross-validation results
print(price_cv_rpart.dum)
plot(price_cv_rpart.dum)

# Create a CART model
price_mod_rpart.dum <- rpart::rpart(price ~ ., data = training.11, cp = 5.7e-07)

# Make predictions
price_pred_rpart.dum = predict(price_mod_rpart.dum, newdata = testing.11)

# model summary                       

# Plot residuals vs. observed values
obs <- training.11$price
fit <- predict(price_mod_rpart.dum)
kernlab::plot(obs, (fit-obs)/obs *100)
...
# Performance measure for regression
caret::postResample(pred = price_pred_rpart.dum, obs = testing.11$price)

# Calculate error on testing set
errTest = (price_pred_rpart.dum - testing.11$price)/testing.11$price * 100
summary(errTest)
...
```
Summary:
  - the use of dummy variables makes the RSME and Rsquared worse!
